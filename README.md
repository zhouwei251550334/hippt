...
3. 正式设计前必须生成 `ppt-content-spec.md`，逐页写明标题、页面内容、建议演讲稿和证据状态，并由用户明确确认。
4. 只有内容确认后，才从使用者维护的 HiPPT 模板资源库中选择新设计模板。
5. 整套 PPT 保持同一视觉体系，但非系列内容页优先使用不同的源版式或衍生构图；只有同类内容、系列页或对照页才复用相同版式。
...
- 标准模式：先确认需求边界，再生成并确认完整的逐页内容说明，之后推荐模板并制作。
- 快速模式：材料完整时一次性生成完整内容说明，但仍须确认后才能选择模板或制作样稿。
- 忠实迁移：保留原稿内容和顺序，先形成逐页内容说明并确认，再迁移到 HiPPT 资源库中的新模板。
...
- 只有内容能力而没有 PPTX 编辑器时，可以完成需求卡、`ppt-content-spec.md` 和素材计划，但不能宣称已交付可编辑 PPTX。
