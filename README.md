...
HiPPT 是一个面向企业、医学和教学场景的跨平台 Agent Skill。它把“统一内容审核、PPTX 模板设计、HTML 互动设计、图片来源、医学证据边界、渲染检查和最终交付”组织成一套可重复执行的演示制作流程，可用于 Codex、Claude Code、WorkBuddy/CodeBuddy，以及其他支持 `SKILL.md` 的应用。

它不是一套固定模板，也不会自动附带第三方 PPT、HTML 主题、字体、模型或图片。每位使用者需要维护自己的资源库，并确保相关资产具备合法使用权限。

![HiPPT 从内容输入、模板选择到质量检查和最终交付的工作流程](docs/hippt-workflow.jpg)

## 为什么需要 HiPPT

通用 AI 制作演示时，经常出现这些问题：

- 直接沿用用户上传旧稿的母版和低质量版式；
- 未确认逐页标题、页面内容和建议演讲稿，就开始套模板；
- 字号过小、层级单一、页面密度失控；
- 反复使用三栏卡片、装饰圆点和随机渐变；
- 用虚构数据、假截图或无来源图片填补页面；
- 医学、法规和市场信息缺少证据状态与来源；
- 文件或网页虽然生成成功，但没有经过逐页内容回查、渲染、交互和可编辑性检查。
...
- 互动产品演示、动态图表、三维、地图和在线演示。

## 核心原则

1. 用户上传的 PPT/PPTX 默认只作为内容来源，提取文字、讲稿、表格文字、图表数据或标签和引用信息。
2. 原稿的母版、版式、配色、字体、背景、装饰、图片和动画默认不继承。
3. PPTX 和 HTML 使用同一套内容审核流程。正式设计前必须生成 `ppt-content-spec.md`，逐页写明标题、页面内容、建议演讲稿和证据状态，并由用户明确确认。
4. 只有内容确认后，PPTX 才进入模板设计，HTML 才进入主题与互动设计。
5. 整套演示保持同一视觉体系，但非系列内容页优先使用不同的版式或衍生构图；只有同类内容、系列页或对照页才复用相同版式。
6. 真实素材、官方资料和可核查网络图片优先；AI 生图仅用于适合的概念性或定制视觉。
7. 医学、法规、市场和公司数据区分已验证、用户提供、分析判断和证据缺口。
8. 标题、正文、图表、页码、来源和讲稿尽量保持可编辑。
9. 每次重要修改后重新检查结构、字体、溢出、渲染、来源和完整性。

## 两种交付格式

选择格式只用于明确最终交付预期，不会改变内容审核标准。两种格式都必须先确认同一份 `ppt-content-spec.md`。

| 维度 | PPTX | HTML |
|---|---|---|
| 推荐场景 | 讲课、比赛、正式汇报、离线文件交付 | 产品演示、数据展示、在线传播、三维和地图 |
| 编辑方式 | PowerPoint/WPS | 浏览器和可选网页编辑器 |
| 互动能力 | 常规动画和链接 | 动态图表、三维、地图、物理、实时参数和批注 |
| 兼容性 | 较高 | 依赖浏览器、GPU、网络和部署环境 |
| 打印与 PDF | 稳定 | 可导出静态版本，但互动会消失 |
| 交付复杂度 | 相对较低 | 需要浏览器、性能、互动、离线和安全测试 |

默认只显示 PPTX 和 HTML 两个选项。PPTX+HTML 双版本仅在用户明确提出时启用；两者共享内容合同，但不要求页面像素级一致。

## 三种制作模式

- 标准模式：先确认需求边界，再生成并确认完整的逐页内容说明，之后进入格式对应的样式设计。
- 快速模式：材料完整时一次性生成完整内容说明，但仍须确认后才能选择 PPTX 模板、HTML 主题或制作样稿。
- 忠实迁移：保留原稿内容和顺序，先形成逐页内容说明并确认，再迁移到新的 PPTX 模板或 HTML 主题。
...
HiPPT 遵循 [Agent Skills 开放规范](https://agentskills.io/specification)：技能目录以 `SKILL.md` 为入口，并使用相对路径引用 `scripts/`、`references/` 和 `assets/`。核心流程可以跨应用复用，但能否直接读写和检查 PPTX，或构建、运行和测试 HTML，取决于宿主应用提供的工具，不能把格式兼容等同于能力完全一致。
...
- PPTX 交付需要读取、编辑和渲染工具，或能够调用 PowerPoint、LibreOffice 等外部工具；
- HTML 交付需要 Web 构建工具和真实浏览器检查能力。

### 能力降级规则

- 有原生 PPT/Slides 工具时优先使用原生能力；Codex 中可使用 `presentations`。
- HTML 模式使用宿主应用的 Web 构建和真实浏览器检查能力；只有用户授权时才进行外部发布。
- 没有同名 `presentations`、网页搜索或生图工具，不代表 Skill 失效；HiPPT 会改用宿主应用实际具备的等价能力。
- 只有内容能力而没有目标格式制作工具时，可以完成需求卡、`ppt-content-spec.md` 和素材计划，但不能宣称已经交付 PPTX 或 HTML 成品。
...
- HTML 主题、布局、互动组件、运行时适配和静态降级资源分别放入 `html/` 对应目录，不要把 PPTX 母版或固定坐标当作 HTML 模板。
...
- 公司和客户品牌包应与通用模板分开管理。
- HTML 主题必须是网页原生布局，不要把 PPTX 页面截图、母版或固定坐标包装成网页模板。
- 为 HTML 主题、互动组件和运行库记录来源、固定版本、许可证、网络依赖和静态降级方案；开源运行库不代表模型、贴图、动画、地图瓦片或数据也可以免费使用。
- 不要把患者资料、客户机密、内部截图或未发布产品素材放入公共资源库。

## 使用示例

```text
用 $hippt 制作一份面向医院管理层的产品介绍，20分钟，先输出逐页内容说明供我确认。
```

Claude Code 可使用：

```text
/hippt 制作一份面向医院管理层的产品介绍，20分钟，先输出逐页内容说明供我确认。
```
...
```text
用 $hippt 忠实保留内容和顺序，先生成逐页内容说明，确认后再选择模板。
```

```text
用 $hippt 制作一份 HTML 互动产品演示。先按与 PPTX 相同的规则审核逐页内容，确认后再设计动态图表和实时参数。
```
...
- 第三方 PPT 模板；
- 第三方 HTML 主题、运行库副本和社区互动资源；
- 字体文件；
- 品牌 Logo 和客户资产；
- 用于 PPT 内容的网络图片或 AI 生成图片；README 工作流说明图除外；
- 示例患者资料和公司内部数据；
- 生成的 PPTX、HTML、PDF、预览图和工作文件。

## 隐私与医学内容

不要把患者可识别信息、医院内部数据或公司机密上传到外部搜索、生图、地图、分析或托管服务。HTML 使用地图、在线素材、统计分析或云端接口前，应核对外部数据流、Cookie、计费、归属标注和账户要求；私钥、服务端密钥与不受限令牌不得写入浏览器代码。医学结论应区分已验证证据、用户提供材料、教学简化和分析判断。HiPPT 是制作与质量控制流程，不替代医学、法律或合规审核。
